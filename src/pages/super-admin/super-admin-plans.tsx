...
export default function SuperAdminPlans() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validFeatures = formData.features.filter(feature => feature.name.trim() !== '');
  ...
  const filteredPlans = plans.filter((plan) => {
    const matchesSearch =
      ...
      plan.features.some(feature => feature.name.toLowerCase().includes(searchValue.toLowerCase()));
      ...
      <div className="flex flex-col min-h-screen w-full flex-1 gap-6 px-7 bg-background text-card-foreground p-5 pt-8 mb-10">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Create, edit, and manage subscription plans for fleet companies.</p>
          </div>
          {refreshing && (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-3 px-4 font-medium text-muted-foreground">Plan Details</th>
                    <th className="text-left py-3 px-4 font-medium text-muted-foreground">Price</th>
                    <th className="text-left py-3 px-4 font-medium text-muted-foreground">Max Vehicles</th>
                    <th className="text-left py-3 px-4 font-medium text-muted-foreground">Features</th>
                    ...
                    <tr>
                      <td colSpan={6} className="text-center py-8">
                        <div className="flex flex-col items-center space-y-2">
                          ...
                          {(!searchValue && filterStatus === "all") && (
                            <Button onClick={openCreateDialog} variant="outline" size="sm" disabled={saving}>
                              {saving ? (
                                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                              ) : (
                                <Plus className="w-4 h-4 mr-2" />
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  ...
                                  <AlertDialogFooter>
                                    <AlertDialogCancel disabled={deleting === plan.id}>Cancel</AlertDialogCancel>
                                    <AlertDialogAction
